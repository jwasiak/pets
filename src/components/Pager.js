...
const Pager = (props) => {
    ...
    const button = (page, current) => {
        ...
        const classes = page === current ? "pagination-link is-current" : "pagination-link";
        ...
        if (page) {
            return (
                <li title="Idź do strony">
                    <a className={classes} aria-label={label} aria-current={aria} onClick={() => goToPage(page)}>
                        {page}
                    </a>
                </li>
            );
    ...
    const previousButton = (page) => {
        return (
            <a className="pagination-previous" onClick={() => goToPage(page)} title="Idź do poprzedniej strony">
                Poprzednia strona
            </a>
        );
    };
    ...
    const nextButton = (page) => {
        return (
            <a className="pagination-next" onClick={() => goToPage(page)} title="Idź do następnej strony">
                Następna strona
            </a>
        );
    };
    ...
    const lastDots =
        [props.totalPages - 2, props.totalPages - 1, props.totalPages].indexOf(props.page) !== -1 ? false : true;
